...
# Configuración del puerto serial para la comunicación con ESP32
PUERTO_SERIAL = '/dev/ttyTHS1'  # Puerto serial en la Jetson
BAUD_RATE = 9600                # Configuración de baudios
...
def enviar_datos_esp(velocidad, direccion):
    try:
        print(f"Enviando datos a ESP32: Velocidad: {velocidad}, Dirección: {direccion}")
    ...
    try:
        while True:
            ...
            if len(centers_l) > 0 or len(centers_r) > 0:
                ...
                # Enviar la corrección de dirección al ESP32
